import VanillaCaret from './vanillaCaret'
...
export default class Undo {
   ...
   */
  save(state) {
    // 插入多个块时，最后一个块为空的block则去掉
    if (state[state.length - 1].type === 'paragraph' && state[state.length - 1].data.text === '') state.pop()

    // 插入一个空的block，不保存
    const prevState = this.stack[this.stack.length - 1].state
    const isEqual = JSON.stringify(state) === JSON.stringify(prevState)
    if (this.stack.length > 1 && isEqual) return
    ...
    const caretIndex =
      state[indexInState] && (
        ...
        state[indexInState].type === 'raw' ||
        state[indexInState].type === 'image') ?
  ...
  getCaretIndex(index) {
    ...
    const { firstChild } = blocks[index]

    const caretBlock = new VanillaCaret(
      this.isRawTool(firstChild) ?
  ...
  async undo() {
    if (this.canUndo()) {
      ...
      if (this.blockWasDeleted(state, nextState)) {
        ...
        if (nextIndex - index > 1) { // 在记录步数最大时间内，连续多次输入内容且有换行,则删除多个block
      ...
      if (block) {
        ...
        if (state[index].type === 'image' || state[index].type === 'button') {
          this.setFoucus(block)
        } else {
          this.setCaretIndex(index, caretIndex)
        }
      }
  ...
  setCaretIndex(index, caretIndex) {
    console.log('setCaretIndex', index, caretIndex)
  ...
  isRawTool(block) {
    return block.className.includes('ce-rawtool')
  }
  ...
  async redo() {
    if (this.canRedo()) {
      ...
      if (this.blockWasDeleted(prevState, state)) {
        ...
        this.caret.setToBlock(index, 'end')
      } else if (this.blockWasDropped(state, prevState)) {
        await this.blocks.render({ blocks: state })
        this.caret.setToBlock(index, 'end')
      }

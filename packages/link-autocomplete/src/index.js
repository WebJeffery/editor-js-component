...
export default class LinkAutocomplete {
  ...
  constructor({ config, api }) {
    ...
    this.api = api
    this.config = config || {}
    this.selection = new SelectionUtils(api)
  ...
  fieldInputHandler(event) {
    ...
    this.typingTimer = setTimeout(async () => {
      ...
      try {
        ...
        notifier.show({
          message: `${this.api.i18n.t(DICTIONARY.searchRequestError)} "${e.message}"`,
          style: 'error'
        })
      }
  ...
  processEnterKeyPressed() {
     ...
     */
    let href = this.nodes.inputField.value.trim()
    ...
    if (!Utils.isUrl(href)) {
      // notifier.show({
      //   message: this.api.i18n.t(DICTIONARY.invalidUrl),
      //   style: 'error'
      // })
  ...
  generateSearchList(items = []) {
    ...
    if (!Utils.isArray(items)) {
      notifier.show({
        message: this.api.i18n.t(DICTIONARY.invalidServerData),
        style: 'error'
      })
  ...
  searchItemPressed(element) {
    ...
    this.insertLinkUrl(href, element.dataset)
  }
  ...
  insertLinkUrl(href) {
    ...
    const u = document.createElement(this.tagName)
    u.href = href
    u.target = '_blank'

    this.selection.removeFakeBackground(u)
  ...
  surround(range) {
     ...
     */
    if (!isLinkSelected) {
      this.selection.setFakeBackground(range)
      this.selection.save()
      /**
  ...
  async searchRequest(searchString) {
    ...
    try {
      ...
      let searchResponse = []
      if (this.config.searchLinkData) {
        searchResponse = await this.config.searchLinkData(searchString)
      } else {
        const searchResponseRaw = await fetch(`${this.searchEndpointUrl}?${queryString}`)
        searchResponse = await searchResponseRaw.json()
      }
  ...
  clear() {
    if (this.selection.isFakeBackgroundEnabled) {
      ...
      this.selection.isFakeBackgroundEnabled = false
    }
  }
}
